...
export function PositionSummary({
  ...
  const totalValuation = owned.some((holding) => holding.valuation === null)
    ? null
    : owned.reduce((sum, holding) => sum + (holding.valuation ?? 0), 0);
  const totalUnrealized = owned.some((holding) => holding.unrealizedProfit === null)
    ? null
    : owned.reduce((sum, holding) => sum + (holding.unrealizedProfit ?? 0), 0);
  const totalRealized = positions.reduce((sum, holding) => sum + holding.realizedProfit, 0);
  const weightedRelative = owned.some((holding) => holding.relativeReturn === null || holding.weight === null)
    ? null
    : owned.reduce((sum, holding) => sum + (holding.relativeReturn ?? 0) * ((holding.weight ?? 0) / 100), 0);
