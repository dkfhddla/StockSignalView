...
| 결측 상태와 계산 상태 | [stock-signal-view-data-model.md](../specs/stock-signal-view-data-model.md), [frontend-user-flow.md](../specs/frontend-user-flow.md), [widget-registry.md](../widget-registry.md) | 백엔드 단위, 프론트엔드 렌더링 | `CALCULATED`, `PARTIAL`, `UNAVAILABLE`, `EMPTY`가 데이터 부족을 성공 상태처럼 숨기지 않는다. | 문구 이해 가능성 점검 |
...
B1 백엔드 참여자와 증거는 백엔드 단위, 서비스, API 통합 검사 행이 소유한다.
B4 프론트엔드 참여자와 증거는 프론트엔드 렌더링 검사와 공유 계약 fixture 행이
소유한다. 이 표기는 설계 추적용이며 새 제품 동작이나 계층 이름을 만들지 않는다.
