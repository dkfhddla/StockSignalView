...
provider는 사용자의 보유 종목 목록을 조회하고, 종목 코드, 종목명, 시장 구분, 보유 수량, 평균 매수가 또는 계산 가능한 원가 근거를 공통 내부 모델로 정규화해야 한다.
...
대시보드는 보유 종목별 현재가, 평균 매수가 대비 현재 수익률, 당일 시장 대비 상대성과, 보유 기간 기준 상대성과, provider명, 마지막 갱신 시각을 표시할 수 있어야 한다.
