...
- Data Binder는 보유 조회 결과의 `lookup_type`과 `target_key`, 가격·지수 조회 결과의 `lookup_type`, `target_key`, `snapshot_role`을 사용해 `lookup_status`를 해당 행이나 계산 입력에 연결하며, 한 대상 또는 한 역할의 조회 실패를 다른 종목·지수·기준 역할에 표시하지 않는다.
