...
- 외부 뉴스/공시 구현 세부
- read-only provider의 provider별 외부 API 엔드포인트, 인증 절차, 원본 응답 형식 세부
...
- provider adapter는 현재가/지수 값의 기준 시각, 당일 기준값의 기준 시각,
  provider에서 가져온 마지막 갱신 시각을 구분해 보존한다. 계산 서비스는
  마지막 갱신 시각을 가격 기준 시각이나 지수 기준 시각으로 대체하지 않는다.
